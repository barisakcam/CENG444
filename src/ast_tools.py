...
class SemanticVisitor(ASTNodeVisitor):
    ...
    def visit_Program(self, program: Program):
        for elem in program.var_decls:
            temp = self.visit(elem)
            #print(temp)
            if temp[0].name in [i.name for i in self.declared]:
                self.multiple_list.append(temp[0])

            for i in temp[1]:
                if i.name not in [j.name for j in self.declared]:
                    self.undeclared_list.append(i)
                    #print(f"Undeclared: {i}")
            self.declared.append(temp[0])
        ...
        return (sorted(self.undeclared_list, key=lambda elem: elem.lineno),
                sorted(self.multiple_list, key=lambda elem: elem.lineno))
        #return (self.undeclared_list, self.multiple_list)
    ...
    def visit_VarDecl(self, vardecl: VarDecl):
        if vardecl.initializer is None:
            ...
            for elem in vardecl.initializer:
                res = res + self.visit(elem)
            #print(res)
            return (vardecl.identifier, res)
        else:
            return (vardecl.identifier, self.visit(vardecl.initializer))

    def visit_FunDecl(self, fundecl: FunDecl):
        self.declared = self.declared + [elem for elem in fundecl.params]
        for elem in fundecl.params:
            if elem.name in [i.name for i in self.params]:
                self.multiple_list.append(elem)
            self.params.append(elem)

        self.visit(fundecl.body)
        self.declared = self.declared[:-len(fundecl.params) or None]
        return []

    def visit_Assign(self, assign: Assign):
        return [assign.identifier] + self.visit(assign.expr) 

    def visit_SetVector(self, setvector: SetVector):
        return [setvector.identifier] + self.visit(setvector.vector_index) + self.visit(setvector.expr)

    def visit_ForLoop(self, forloop: ForLoop):
        initializer = [] if forloop.initializer is None else self.visit(forloop.initializer)
        condition = [] if forloop.condition is None else self.visit(forloop.condition)
        increment = [] if forloop.increment is None else self.visit(forloop.increment)
        block = [] if forloop.body is None else self.visit(forloop.body)
        return initializer + condition + increment + block
    ...
    def visit_Block(self, block: Block):
        ...
        for elem in block.var_decls:
            temp = self.visit(elem)
            #print(temp)
            if temp[0].name in [i.name for i in local_declared]:
                self.multiple_list.append(temp[0])
        ...
        self.declared = self.declared[:-len(block.var_decls) or None]
    ...
    def visit_IfElse(self, ifelse: IfElse):
        if ifelse.else_branch is None:
            return self.visit(ifelse.condition) + self.visit(ifelse.if_branch)
        else:
             return self.visit(ifelse.condition) + self.visit(ifelse.if_branch) + self.visit(ifelse.else_branch)
    ...
    def visit_GetVector(self, getvector: GetVector):
        return [getvector.identifier] + self.visit(getvector.vector_index)
